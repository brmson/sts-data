...
class SkipThought(Embedder):
    ...
    * config['skipthoughts_uni_bi'] = 'uni' or 'bi' or 'combined'; Two different 
    ...
    def __init__(self, c=None):
        """Load precomputed model, from c["skipthoughts_datadir"]."""
        if not c:
            c = {}
        self.c = c
        ...
        if self.uni_bi in ("uni", "bi"):
            ...
            raise KeyError("config['uni_bi'] has invalid value. Possible values: 'uni', 'bi', 'combined'")
    ...
    def batch_embedding(self, sentences):
        ...
        embeddings = self.encode(self.st, new_sentences, verbose=False)
    ...
    def map_tokens(self, tokens, ndim=2):
        ...
        if self.uni_bi == 'combined':
            ...
            raise ValueError("uni_bi has invalid value")
